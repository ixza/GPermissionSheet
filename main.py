...
wks = gc.open_by_key(config.WRITE_SPREADSHEET_ID).sheet1


from formatSheets import formatSheets
       
service.spreadsheets().batchUpdate(spreadsheetId=config.WRITE_SPREADSHEET_ID, body=formatSheets.clearAll).execute() #CLEARS THE FORMAT AND VALUES OF WHOLE SHEET
service.spreadsheets().batchUpdate(spreadsheetId=config.WRITE_SPREADSHEET_ID, body=formatSheets.enteredSheets).execute()
...
service.spreadsheets().batchUpdate(spreadsheetId=config.WRITE_SPREADSHEET_ID, body=formatSheets.adjustColumnWidth).execute() #ADJUST COLUMNS WIDTH